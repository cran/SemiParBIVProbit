SemiParBIVProbit <- function(formula, data = list(), weights = NULL, subset = NULL,
  ...
  cta <- data.frame( c(opc),
                     c(1,3,23,13,33,6,26,16,36,4,24,14,34,5,55,56,2) 
                     )                   
if(BivD %in% BivD2){
...
######  
  ...
  ass.s <- co 
  ass.s <- sign(ass.s)*ifelse(abs(ass.s) > 0.85, 0.85, abs(ass.s))
...
if(missing(parscale)) parscale <- 1   
  ...
  VC <- list(X1 = X1, inde = inde,my.env=my.env,
             X2 = X2, 
             X3 = X3,
             X4 = X4, 
             X5 = X5, 
             ...
             BivD2 = BivD2, cta = cta, ct = ct, zerov = -10) # original n only needed in SemiParBIVProbit.fit
...
if(gamlssfit == TRUE && margins[2] != bl){ 
  ...
  SemiParFit.p <- SemiParBIVProbit.fit.post(SemiParFit = SemiParFit, Model = Model, VC = VC, GAM)
