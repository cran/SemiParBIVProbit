...
cont2par <- c("N","GU","rGU","LO","LN","WEI","iG","GA") 
...
if(!is.null(VC$gp3)){
 ...
 EQ4P <- EQ5P <- EQ6P <- NULL 
    ...
    EQ4P <- adiag(ma4, S4)
	    ...
	    if(VC$l.sp6 == 0) EQ6P <- adiag(ma6)    
	    
	    if(VC$l.sp6 != 0){
	    ind <- (l.sp1 + VC$l.sp2 + VC$l.sp3 + VC$l.sp4 + VC$l.sp5 + 1):(l.sp1 + VC$l.sp2 + VC$l.sp3 + VC$l.sp4 + VC$l.sp5 + VC$l.sp6)
    ...
    if( eq1 == "no" ) {EQ4P <- NULL; S.h <- adiag(EQ2P, EQ3P, EQ4P, EQ5P, EQ6P)} else S.h <- adiag(EQ1P, EQ2P, EQ3P, EQ4P, EQ5P, EQ6P) 
