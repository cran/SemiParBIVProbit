NP.qu <- function(SemiParFit, BivD, nC, nu, gev.eq1, gev.eq2, shape1, shape2, y1.y2, y1.cy2, cy1.y2, cy1.cy2, cy1, X1, X2, X1.d2, X2.d2, pPen1=NULL, pPen2=NULL, qu.mag=NULL, gp1, gp2, 
  ...
  teta.st <- params[X1.d2+X2.d2+2*K+1]
    ...
    if(BivD=="F")                  teta <- teta.st 
    if(BivD %in% c("C0", "C180") ) teta <- exp(teta.st)
    if(BivD %in% c("C90","C270") ) teta <- -( exp(teta.st)  ) 
    if(BivD %in% c("J0", "J180") ) teta <- exp(teta.st) + 1 
    if(BivD %in% c("J90","J270") ) teta <- -( exp(teta.st) + 1  ) 
