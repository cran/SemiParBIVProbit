OR <- function(x, nm.end, E = TRUE, treat = TRUE, type = "bivariate", ind = NULL, delta = FALSE, 
...
if(x$gamlssfit == FALSE && type == "univariate" && x$margins[2] %in% c(m2,m3)) stop("You need to fit the univariate model to obtain the OR. Refit the model and set gamlssfit = TRUE.")
...
C.01  <- p.etn - C.11
C.00  <- 1 - ( pmax( BiCDF(p.int0, p.etn, x$nC, ass.p) , epsilon ) + C.10 + (p.etn - pmax( BiCDF(p.int0, p.etn, x$nC, ass.p) , epsilon )) )
 

est.ATso <- (   (C.11/p.etn) * (C.00/(1-p.etn))  )  /  ( (C.01/p.etn) * (C.10/(1-p.etn))    )          
...
for(i in 1:n.sim){ 
 ...
 C.01  <- p.etns[,i] - C.11
 C.00  <- 1 - ( pmax( BiCDF(p.int0s[,i], p.etns[,i], x$nC, ass.ps[,i], test = FALSE) , epsilon ) + C.10 + (p.etns[,i] - pmax( BiCDF(p.int0s[,i], p.etns[,i], x$nC, ass.ps[,i], test = FALSE) , epsilon )) )
  
 
 est.ATb[i] <- mean(   ( (C.11/p.etns[,i])*(C.00/(1-p.etns[,i])) / (C.01/p.etns[,i])*(C.10/(1-p.etns[,i])) ), na.rm = TRUE   )
...
if( is.null(x$X3) ) ass.ps <- matrix(ass.ps,  ncol = n.sim, nrow=dim(eti0s)[1], byrow = TRUE)           
 ...
 C.01  <- p.etns - C.11
 C.00  <- 1 - ( pmax( BiCDF(p.int0s, p.etns, x$nC, ass.ps, test = FALSE) , epsilon ) + C.10 + (p.etns - pmax( BiCDF(p.int0s, p.etns, x$nC, ass.ps, test = FALSE) , epsilon )) )
  
 
 est.ATb <- colMeans(   ( (C.11/p.etns)*(C.00/(1-p.etns)) / (C.01/p.etns)*(C.10/(1-p.etns)) ), na.rm = TRUE   )
...
if(x$VC$margins[2] %in% m3){ if(is.null(x$X5) ) nus <- matrix(nus, ncol = n.sim, nrow=dim(etnos)[1],byrow = TRUE) }  
 ...
 fy10.y2S[, j] <- apply(h, MARGIN = 2, FUN = mean, na.rm=TRUE) # can i remove this?             
 fy1.y2S[, j]  <- 1 - fy10.y2S[, j] # apply(1 - h, MARGIN = 2, FUN = mean, na.rm=TRUE)
...
fy1.y2[i]  <- mean(p1)
fy10.y2[i] <- 1 - fy1.y2[i] # mean(1-p1)
...
###########
 ...
 fy10.y2S[, j] <- 1 - fy1.y2S[, j] # apply(1 - p1s, MARGIN = 2, FUN = mean, na.rm=TRUE)  
...
#diffEf  <- (fy1.y2[2:ly2] / fy1.y2[1:(ly2-1)]) * (fy10.y2[1:(ly2-1)] / fy10.y2[2:ly2])
est.AT  <- mean(diffEf)
...
#for(i in 1:n.sim) { diffEfS[i,]  <- (fy1.y2S[i,2:ly2] / fy1.y2S[i,1:(ly2-1)]) * (fy10.y2S[i,1:(ly2-1)] / fy10.y2S[i,2:ly2])
#                    est.ATb[i] <- mean(diffEfS[i,]) } 
