...
estfun <- cbind( c(x$fit$dl.dbe1)*x$X1, c(x$fit$dl.dbe2)*X2m, c(x$fit$dl.drho)*mul )

}



if( x$Model == "BPO0" || ( x$Model == "B" && !is.null(x$VC$theta.fx) ) ) estfun <- cbind( c(x$fit$dl.dbe1)*x$X1, c(x$fit$dl.dbe2)*x$X2 )
