startSS <- function(gam1, gam2, formula.eq2, data, gamma, p.weights, inde, l.sp1, l.sp2){
  			       ...
  			       environment(formula.eq2.1) <- environment(NULL)
                               ...
                               if(l.sp1!=0 && l.sp2!=0) l <- as.matrix(data.r[,2:(ols$smooth[[1]]$first.para-1)]) else l <- as.matrix(data.r[,-1])
                               fw <-  paste(names(data.r)[1],"~ l",sep="")
                               if(l.sp1!=0 && l.sp2!=0) for(i in 1:length(ols$smooth)) fw <- paste(fw,"+",ols$smooth[[i]]$label,sep="")
                               fw <- as.formula(fw)
	                       gam2.1 <- suppressWarnings(try(gam(fw, binomial(link="probit"), gamma=gamma, weights=p.weights[inde], data=data.r),silent=TRUE))
                               ...
                               if(l.sp1!=0 && l.sp2!=0) names(gam2.1$coefficients)[1:(ols$smooth[[1]]$first.para-1)] <- names(ols$coefficients)[1:(ols$smooth[[1]]$first.para-1)] else names(gam2.1$coefficients) <- names(ols$coefficients) 
