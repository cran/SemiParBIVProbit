...
if(VC$margins[2] %in% VC$m3)  dHs  <-      distrHs(y2, eta2, sigma2, sigma2.st, nu, nu.st, margin2=VC$margins[2], naive = TRUE)
if(VC$margins[2] %in% VC$m3d) dHs  <- distrHsDiscr(y2, eta2, sigma2, sigma2.st, nu, nu.st, margin2=VC$margins[2], naive = TRUE, y2m = VC$y2m)
...
if( VC$margins[1] %in% c(VC$m2,VC$m2d,VC$m3) ) sigma2.st <- VC$X4%*%params[(VC$X2.d2+1):(VC$X2.d2+VC$X4.d2)]
if( VC$margins[1] %in% c(VC$m1d) )             sigma2.st <- VC$X3%*%params[(VC$X2.d2+1):(VC$X2.d2+VC$X3.d2)]

if(!is.null(VC$X4)){
if( VC$margins[1] %in% c(VC$m3)        ) nu.st <- VC$X6%*%params[(VC$X2.d2+VC$X4.d2+1):(VC$X2.d2+VC$X4.d2+VC$X6.d2)] 
if( VC$margins[1] %in% c(VC$m2,VC$m2d) ) nu.st <- VC$X5%*%params[(VC$X2.d2+VC$X4.d2+1):(VC$X2.d2+VC$X4.d2+VC$X5.d2)] 
if( VC$margins[1] %in% c(VC$m1d) )       nu.st <- VC$X4%*%params[(VC$X2.d2+VC$X3.d2+1):(VC$X2.d2+VC$X3.d2+VC$X5.d2)] 
                   }
}
...
if(VC$margins[2] %in% VC$m3)  dHs <-      distrHs(respvec$y2, eta2, sigma2, sigma2.st, nu, nu.st, margin2=VC$margins[2], naive = TRUE)
if(VC$margins[2] %in% VC$m3d) dHs <- distrHsDiscr(respvec$y2, eta2, sigma2, sigma2.st, nu, nu.st, margin2=VC$margins[2], naive = TRUE, y2m = VC$y2m)
