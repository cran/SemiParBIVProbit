bprobSSH <- function(params, dat, dat1, dat2, p.weights=p.weights, dat1p=NULL, dat2p=NULL, X1.d2, X2.d2, S=NULL, gam1, gam2, fp, K=NULL, n=NULL, N=NULL, cuid=NULL, uidf=NULL, masses=NULL){
    ...
    l.par <- p.weights*(y1*log(PHI2) + cy1*log(p0))  
    
    dl.dbe1 <- p.weights*(y1*q1*g1/PHI2 - d.n1*cy1/p0)
    dl.dbe2 <- p.weights*(y1*q2*g2/PHI2)
    dl.drho <- p.weights*(y1*q1*q2*phi2/PHI2*drh.drh.st)
    
    d2l.be1.be1 <- -p.weights*(y1*(-w1*g1 - corr.sq*phi2 - g1^2/PHI2)/PHI2 - CY1)  
    d2l.be2.be2 <- -p.weights*y1*(-w2*g2 - corr.sq*phi2 - g2^2/PHI2)/PHI2 
    d2l.be1.be2 <- -p.weights*y1*(q1*q2)/PHI2*(phi2 - g1*g2/PHI2) 
    d2l.be1.rho <- -p.weights*y1*(corr.sq*delta*v1 - w1 - g1/PHI2)*q2*phi2/PHI2*drh.drh.st
    d2l.be2.rho <- -p.weights*y1*(corr.sq*delta*v2 - w2 - g2/PHI2)*q1*phi2/PHI2*drh.drh.st
    d2l.rho.rho <- -p.weights*(y1*( phi2/PHI2*( delta^2*corr.sq*( 1 - delta^2*(w1^2+w2^2 - 2*corr.sq*w1*w2) ) + delta^2*w1*w2 - phi2/PHI2 )*drh.drh.st^2 + q1*q2*phi2/PHI2*drh.drh.st2 ) )
