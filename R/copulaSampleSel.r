...
#######################################################################
  ...
  ass.s <- sign(co)*ifelse(abs(co) > 0.9, 0.9, abs(co))
...
L <- list(fit = SemiParFit$fit, dataset = dataset, formula = formula,
          ...
          l.flist = l.flist, v1 = v1, v2 = v2, triv = FALSE, univar.gamlss = FALSE, BivD2 = BivD2, dof = dof, dof.a = dof)
